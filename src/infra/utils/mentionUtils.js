...
import { Logger } from "src/infra/reporting/index";
...
const getTrimmedTextWithMentionEntities = (text, mentions) => {
  ...
  mentions.reverse().forEach(mention => {
    const mentionText = `@${mention.entity.entityType}:${mention.entity
      .objectID || mention.entity.id}`;
...
const enrichInputTextWithMentionsList = ({
  text,
  mentions,
  isWithBrackets
}) => {
  ...
  while (mentionIndex > -1) {
    ...
    if (mention) {
      ...
      Logger.error(
        `mention entity was not found in ${enrichedText} on index ${mentionIndex}`
      );
    }
