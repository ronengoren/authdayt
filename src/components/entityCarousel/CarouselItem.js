...
import { View, Text, Image, BreadCrumbs } from "src/components/basicComponents";
import { EntityAction, EntityImagePlaceholder } from "src/components/entity";
...
import { getFormattedDateAndTime } from "src/infra/utils/dateTimeUtils";
import { toCurrency } from "src/infra/utils/stringUtils";
...
class CarouselItem extends Component {
  ...
  imageSizeMap = {
    ...
    [CarouselItem.sizes.SMALL_UNIFIED_LOOK]: EntityImagePlaceholder.sizes.MEDIUM
  };
  ...
  renderEntityAction = () => {
    ...
    return (
      <EntityAction
        size={
          size === CarouselItem.sizes.SMALL_UNIFIED_LOOK ? "tiny" : "medium"
        }
  ...
  navigateToEntityPage = ({ showKeyboard = false }) => {
    ...
    navigationService.navigate(screenName, {
      entityId: id,
      originType: originTypes.SUGGESTED,
      groupType,
      showKeyboard
    });
  };
