...
//     if (state.searchQueryProp !== props.searchQuery && searchQuery && searchQuery.length) {
...
//     const { searchMode, searchAddressMode, searchAddressData, searchQuery, communityId, search, searchAddress, clearSearch, clearSearchAddress, destinationTagName } = this.props;

//     if (searchAddressMode) {
//       if (text && text.length) {
//         const trimmedText = text.trim();
//         const { isNeighborhoods, country, types, prefix, coordinates } = searchAddressData;
...
//           search({ query: trimmedText, page: 0, communityId, destinationTagName, searchType: searchTypes.GENERAL, entityTypeFilter: entityTypes.USER });
//           const { params = {} } = navigationService.getCurrentRouteName({ withParams: true });
//           if (params.withPeopleSearch) {
//             search({ query: trimmedText, page: 0, communityId, destinationTagName, searchType: searchTypes.PEOPLE, singleEntityType: entityTypes.USER });
//           }
//         }
//       } else {
...
//   searchQuery: ownProps.searchMode && !ownProps.searchAddressMode && state.search.searchStack.length ? state.search.searchStack[state.search.searchStack.length - 1].query : ''
// });
