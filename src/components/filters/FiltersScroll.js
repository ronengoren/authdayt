...
import { remove } from "src/infra/utils";
  ...
  getFilterName = ({ filterName }) => {
    ...
    if (!filters[filterName]) {
      if (
        ...
        return `${I18n.t(`filters.age.header`)}: ${filters.minAge ||
          AgeFilter.values.min} - ${filters.maxAge || AgeFilter.values.max}`;
      }
    ...
    switch (filterName) {
      ...
      case filterTypes.PRICE: {
        return `${I18n.t(`filters.price.header`)}: ${filters.price.currentMin ||
