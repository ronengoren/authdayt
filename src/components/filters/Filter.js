...
import { Text, View, ScrollView } from "src/components/basicComponents";
import { daytColors, uiConstants } from "src/vars";
import { filterTypes } from "src/vars/enums";
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    borderTopRightRadius: 15,
    borderBottomWidth: 1,
    borderBottomColor: daytColors.paleGreyTwo
  },
  filtersWrapper: {
    paddingHorizontal: 15,
...
class Filter extends React.Component {
  ...
  render() {
    const { closeFilter, filterType } = this.props;
    const {
            ...
            <Animated.View style={{ transform: [{ translateY }] }}>
              ...
              >
                <View style={styles.header}>
                  <Text size={16} lineHeight={21} color={daytColors.b30} bold>
                    {headerText}
                  </Text>
                  <TouchableOpacity
  ...
  componentDidMount() {
    ...
    if (filterType === filterTypes.HOODS) {
  ...
  handleSubmitPressed = () => {
    ...
    const afterAnimationAction = shouldClearFilter
      ...
      : () =>
          applyFilter(
            this.formatFilterValues({ state: this.state, filterType })
          );
