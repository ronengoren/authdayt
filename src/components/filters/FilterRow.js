import React, { Component } from "react";
import PropTypes from "prop-types";
import { StyleSheet, TouchableOpacity } from "react-native";
import { Checkbox, Text } from "src/components/basicComponents";
import { daytColors } from "src/vars";

const styles = StyleSheet.create({
  wrapper: {
    flex: 1,
    flexDirection: "row",
    alignItems: "center",
    height: 65
  },
  borderTop: {
    borderTopWidth: 1,
    borderTopColor: daytColors.b90
  },
  checkbox: {
    marginRight: 15
  }
});

class FilterRow extends Component {
  render() {
    const {
      index,
      action,
      isActive,
      text,
      data,
      selectedItems,
      ignoredItems
    } = this.props;

    if (ignoredItems && ignoredItems.includes(data.id)) {
      return null;
    }

    let adjustedAction = action;
    let isActiveAdjusted = isActive;
    let adjustedText = text;
    if (data) {
      adjustedAction = () => action({ id: data.id, name: data.name });
      isActiveAdjusted = selectedItems.findIndex(item => item === data.id) > -1;
      adjustedText = data.name;
    }

    return (
      <TouchableOpacity
        onPress={adjustedAction}
        activeOpacity={0.5}
        style={[styles.wrapper, !!index && styles.borderTop]}
      >
        <Checkbox
          value={isActiveAdjusted}
          size="small"
          selectedBackgroundColor={daytColors.azure}
          style={styles.checkbox}
        />
        <Text size={16} lineHeight={20} color={daytColors.b30}>
          {adjustedText}
        </Text>
      </TouchableOpacity>
    );
  }
}

FilterRow.propTypes = {
  index: PropTypes.number,
  action: PropTypes.func,
  isActive: PropTypes.bool,
  text: PropTypes.string,
  data: PropTypes.object,
  selectedItems: PropTypes.array,
  ignoredItems: PropTypes.array
};

export default FilterRow;
