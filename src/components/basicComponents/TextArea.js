...
class TextArea extends React.Component {
  ...
  //       withMentions
  //     } = this.props;
  //     const isNewMentionAdded =
  //       mentionsList.length > prevProps.mentionsList.length;
  ...
  //         newMentionsList,
  //         changedText
  //       } = mentionUtils.getUpdatedMentionListAfterMentionAdd({
    ...
    if (withMentions) {
      ...
      if (mentionsList.length) {
        const {
          newMentions,
          changedText
        } = mentionUtils.getUpdatedMentionsListAfterTextChange({
      ...
      const cursorPosition = this.selectionAfterChange
        ? this.selectionAfterChange.end
        : -1;
      ...
      if (mentionText) {
        const isUserRemovingText =
          !searchMentions.query ||
          mentionText[1].length < searchMentions.query.length;
        if (searchMentions.resultsHits !== 0 || isUserRemovingText) {
          querySearchMentions(mentionText[1], 0, communityId);
