...
class Slider extends React.Component {
  ...
  renderSlider = () => {
    ...
    if (this.platform === "android" && withAndroidIntervals) {
      return (
        <ViewPager
          ref={scroll => {
          ...
          onPageSelected={this.handleMomentumScrollEnd}
          key={children.length}
          style={styles.wrapperAndroid}
          peekEnabled
          pageMargin={sliderWidth + slidePadding - width}
        >
          {this.renderSlides()}
        </ViewPager>
      );
      ...
      return (
        <ScrollView
          ...
          snapToInterval={sliderWidth + slidePadding} // iOS only. React-native v0.53.0-rc1 supports it.
          snapToAlignment="start" // iOS Only
  ...
  updateOffset = () => {
    this.setState({
      offsetX: this.state.currentSlide * this.props.sliderWidth
    });
  };
  ...
  scrollToNextSlide = () => {
    ...
    const { sliderWidth, withAndroidIntervals } = this.props;
...
Slider.propTypes = {
  ...
  withAndroidIntervals: PropTypes.bool,
