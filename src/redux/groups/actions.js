...
import { groupRoleTypes, groupPrivacyType, groupType } from "src/vars/enums";
import { analytics } from "src/infra/reporting";
import { getKeyByValue } from "src/infra/utils";
import { joinArrayToString } from "src/infra/utils/stringUtils";
...
export const getGroup = ({ groupId }) => async dispatch => {
  const res = await dispatch(
    apiQuery({
      query: { domain: "groups", key: "getGroup", params: { groupId } }
    })
  );
  const { data } = res;
  const payload = {
...
export const dehighlightEntity = ({
  groupId,
  entityId,
  entityType
}) => dispatch => {
