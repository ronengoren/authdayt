...
// import { sortedIndexBy, sortedLastIndexBy, pull, get, merge, cloneDeep } from 'lodash';
// import { entityTypes } from '/vars/enums';
// import { ADD_ENTITIES } from '/redux/normalizer';
// import { SAVE, UNSAVE } from '/redux/themes/actions';
// import { getUpdatedVotesForItem } from '/redux/utils/common';
...
//             draft.byId[key] = merge({}, cloneDeep(state.byId[key] || {}), lists[key]);
//             if (lists[key].items) {
...
//         const { id } = action.payload;
//         draft.items[id] = { ...action.payload, comments: action.payload.comments.map((comment) => comment.id) };
//         break;
//       }
...
//             draft.byId[listId].totalSaves = saved ? previousTotalSaves + 1 : previousTotalSaves - 1;
//             if (saved) {
//               draft.byId[listId].savers.unshift(actor);
//             } else {
//               const saverIndex = state.byId[listId].savers.findIndex((item) => item.id === actor.id);
//               draft.byId[listId].savers.splice(saverIndex, 1);
//             }
//           }
//         }

//         if (entityType === entityTypes.LIST_ITEM) {
//           const { parentId: listId, entityId: listItemId, saved } = action.payload;

//           const listItemsData = get(state, `byId[${listId}].items.data`);
//           if (listItemsData) {
//             const index = listItemsData.findIndex((item) => item.id === listItemId);
//             if (index > -1) {
...
//           const index = listItemsData.findIndex((item) => item.id === listItemId);
//           if (index > -1) {
//             const updatedItem = getUpdatedVotesForItem({ item: listItemsData[index], voted, voter });

//             if (sortByVotes) {
//               draft.byId[listId].items.data.splice(index, 1);
//               const newItemsData = [...listItemsData.slice(0, index), ...listItemsData.slice(index + 1)];
//               let insertedIndex;

//               if (voted) {
//                 insertedIndex = sortedLastIndexBy(newItemsData, updatedItem, (item) => -item.totalVotes);
//               } else {
//                 insertedIndex = sortedIndexBy(newItemsData, updatedItem, (item) => -item.totalVotes);
//               }
//               draft.byId[listId].items.data.splice(insertedIndex, 0, updatedItem);
...
//           } else {
//             const voterIndex = state.items[listItemId].voters.findIndex((v) => v.id !== voter.id);
...
//           const index = newState.savedItems.data.findIndex((item) => item.id === listItemId);
//           if (index > -1) {
...
//             } else {
//               const voterIndex = state.savedItems.data[index].voters.findIndex((v) => v.id !== voter.id);
...
//           draft.byId[listId].totalItems += 1;
//         }
//         break;
//       }
...
//         const { listId, listItemId, totalVotes } = action.payload;
...
//             draft.byId[listId].items.data.splice(index, 1);
//             draft.byId[listId].totalItems -= 1;
//             draft.byId[listId].totalVotes = totalVotes;
//           }
//         }
...
//           const shouldAddCommentToListItem = contextEntity.type === entityTypes.LIST_ITEM;
//           draft.items[topContextEntity.id].totalComments = parseInt(state.items[topContextEntity.id].totalComments, 0) + 1;
...
//           const listItemsData = get(state, `byId[${listId}].items.data`);
//           if (listItemsData) {
//             const listItemIndex = listItemsData.findIndex((item) => item.id === topContextEntity.id);
//             if (listItemIndex > -1) {
...
//           const tempIndex = state.items[contextEntity.id].comments.findIndex((commentId) => commentId === tempId);
...
//         const { topContextEntity, commentId, numberOfCommentsDeleted } = action.payload;

//         if (state.items[topContextEntity.id]) {
//           draft.items[topContextEntity.id].comments = pull(state.items[topContextEntity.id].comments, commentId);
//           draft.items[topContextEntity.id].totalComments = parseInt(state.items[topContextEntity.id].totalComments, 0) - numberOfCommentsDeleted;
...
//             const listItemIndex = listItemsData.findIndex((item) => item.id === topContextEntity.id);
//             if (listItemIndex > -1) {
//               draft.byId[listId].items.data[listItemIndex].totalComments -= numberOfCommentsDeleted;
//             }
//           }
//         }
//         break;
...
//       case actions.MARK_USED_LIST_ITEM_CTA: {
//         const { listId } = action.payload;

//         const index = state.listItemCtaPosts.data.findIndex((list) => list.id === listId);
//         if (index > -1) {
//           draft.listItemCtaPosts.data[index].isUsed = true;
//         }
//         break;
//       }
