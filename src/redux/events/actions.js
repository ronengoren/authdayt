...
import { updateSharedEventPostRSVP } from "src/redux/feed/actions";
import { setRSVP as setGroupEventRSVP } from "src/redux/groups/actions";
import { analytics } from "src/infra/reporting";
import { getKeyByValue, get } from "src/infra/utils";
import { eventPrivacyTypes } from "src/vars/enums";
...
export const getEvent = ({ eventId }) => async dispatch => {
  const res = await dispatch(
    apiQuery({
      query: { domain: "events", key: "getEvent", params: { eventId } }
    })
  );
  const { data } = res.data;
  const payload = {
