  ...
  produce(state, draft => {
    switch (action.type) {
      ...
      case actions.EVENT_CREATE: {
        ...
        draft.byId[eventId] = event;
        draft.myEvents.data.unshift(eventId);
        break;
      }
      ...
      case actions.EVENT_SET_RSVP: {
        ...
        if (state.suggestedEventsCarousel.data) {
          const eventCarouselIndex = state.suggestedEventsCarousel.data.findIndex(
            event => event.id === eventId
          );
          if (eventCarouselIndex !== -1) {
            const didUserRSVP = [rsvpStatuses.YES, rsvpStatuses.NO].includes(
              rsvpStatus
            );
            if (didUserRSVP) {
              draft.suggestedEventsCarousel.data = [
                ...state.suggestedEventsCarousel.data.slice(
                  0,
                  eventCarouselIndex
                ),
                ...state.suggestedEventsCarousel.data.slice(
                  eventCarouselIndex + 1
                )
              ];
            }
          }
