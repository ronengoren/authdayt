...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    case actions.SEARCH_MENTION_START: {
      ...
      const hadResultsInPreviousSearch = lastSearchResults
        ? !!lastSearchResults.length
        : true;
      const isQuerySmaller =
        !state.searchMentions.query ||
        state.searchMentions.query.length > action.payload.query.length;
      ...
      return {
        ...
        searchMentions: {
          ...
          isSearching:
            state.isSearching || hadResultsInPreviousSearch || isQuerySmaller,
          ...
          searchMentions: {
            results:
              action.payload.page > 0
                ? state.searchMentions.results.concat(action.payload.results)
