...
import { updateProfile, getProfile } from "src/redux/profile/actions";
...
import { get, isAppAdmin } from "src/infra/utils";
import {
  getVersionNumber,
...
const styles = StyleSheet.create({
  ...
  rightText: {
    ...
    letterSpacing: 0.4,
    textAlign: "right",
    color: daytColors.secondaryBlack
  },
  settingRowArrow: {
    lineHeight: 22,
    paddingLeft: 5
...
class Settings extends React.Component {
        ...
        <ScrollView style={styles.innerContainer}>
          ...
          >
            ...
            <View style={styles.languageRightSection}>
              <Text style={styles.rightText}>
                {Settings.getLanguageName(form.language)}
              </Text>
              <DaytIcon
                name="right-arrow"
                size={20}
          ...
          {isAppAdmin(data.user) && (
            <View>
              ...
              {this.renderFeatureFlagRow({
                translationKey: "community_feed",
                stateKey: "enableCommunityFeed"
              })}
              {this.renderFeatureFlagRow({
                translationKey: "personalized_feed",
                stateKey: "enablePersonalizedFeed"
              })}
              {this.renderFeatureFlagRow({
                translationKey: "nationality_group",
                stateKey: "enableNationatityGroup"
              })}
              <View style={styles.settingsChapterSeparator} />
            </View>
          )}
          ...
          {this.renderSettingsRow({
            ...
            rightText: versionNumber.appVersion
          })}
          ...
          {/*
          Removing for now until we make the flow better
          this.renderSettingsRow({ translationKey: 'delete_account_button', onPress: this.openDeleteAccountMail, isWithArrow: true })
          */}
          {this.renderSettingsRow({
            translationKey: "sign_out_button",
            onPress: logout,
            textProps: { size: 15, lineHeight: 22, color: daytColors.red }
          })}
        </ScrollView>
      </View>
    );
  ...
  renderNotificationSettingsRow = ({
    ...
    return (
      <View style={styles.settingsLine}>
        ...
        <Switch
          active={form[settingsSection][stateKey]}
          onChange={val =>
  ...
  renderEmailRow = () => {
    ...
    if (isEmailVerified) {
      ...
      secondRowColor = daytColors.green;
      ...
      >
        ...
        {!!versionUpdateMessage && (
          <View style={styles.settingsRowSecondLine}>
            <Text size={15} color={daytColors.pinkishRed}>
              {versionUpdateMessage}
            </Text>
          </View>
        )}
  ...
  navigateToTermsAndConditions = () => {
    navigationService.navigate(screenNames.WebView, {
      url: "https://www.ronengoren.com/terms"
    });
  };

  navigateToPrivacyPolicy = () => {
    navigationService.navigate(screenNames.WebView, {
      url: "https://www.ronengoren.com/privacy"
    });
  };
    ...
    this.setState(
      {
        form: {
          ...form,
          [settingsSection]: { ...form[settingsSection], [prop]: val }
        }
      },
      () => {
        const { form } = this.state;
  ...
  handleLanguagePress = () => {
    ...
    const data = {
      options: ["en", "he", "fr", "es"].map(language => ({
        id: language,
        text: Settings.getLanguageName(language),
        shouldClose: true,
        action: () => this.handleLanguageChange(language)
      })),
  ...
  handleLanguageChange = newLanguage => {
    ...
    this.setState({ form: newForm }, () => {
      ...
      I18n.changeLocalization({ locale: newLanguage });
